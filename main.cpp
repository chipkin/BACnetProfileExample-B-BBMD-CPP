...
// to 389020 and can be overridden on the command line with --deviceID. Keep it
...
// instance is runtime-configurable via --deviceID, but DEVICE_NAME is not - so
// it is easy to ship two units, configure their instances correctly, and still
// have BOTH announce Object_Name "Rainbow" - a spec violation, and a hard BTL
// failure. In a real product Object_Name must be per-unit configurable too:
// derive it from a serial number, DIP switches, a config file, or add a
// --deviceName argument.
...
// accepts the command only if it matches. "" = no password required, and accepts
// any request. Set this to your device's secret before shipping - but see the
// NOTE ON SECURITY next to DeviceCommunicationControl() below: this password
// crosses the wire in plaintext and is a guard against accidents, not a security
// boundary.
